...
FIX: ExpenseLine no longer has a 'contact' field (not in Merge.dev schema).
"""
...
def generate_operating_expenses(
    ...
    Returns dict: { 'expenses': [...], 'monthly_opex': {month: {cat: amount}} }
    """
    rng = random.Random(seed)

    account_map  = {a.name: a for a in accounts}
    bank_account = account_map.get("Operating Checking Account")
    ...
    for month_num in range(1, NUM_MONTHS + 1):
        ...
        for category, base_amount in OPERATING_EXPENSES.items():
            ...
            elif category == "Travel & Entertainment":
                amount *= rng.uniform(0.6, 1.5)
            elif category == "Office Supplies & Misc":
            ...
            if category == "SaaS Subscriptions":
                for tool_name, tool_cost in SAAS_TOOLS.items():
                    ...
                    lines.append(ExpenseLine(
                        ...
                        # FIX: no 'contact' field at line level
                    ))
            ...
            pay_day  = 1 if category == "Office / Co-working (WeWork)" else rng.randint(1, 25)
