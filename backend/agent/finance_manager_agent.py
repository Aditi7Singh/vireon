...
=====================
Operational finance manager agent for budgets, close, approvals, audit, and consolidation.
"""
...
from agent.prompts import build_finance_manager_system_prompt
...
class FinanceManagerState(TypedDict):
...
def classify_node(state: FinanceManagerState) -> FinanceManagerState:
...
def agent_node(state: FinanceManagerState) -> FinanceManagerState:
    llm = get_llm(thinking=should_use_thinking_mode(state["query_type"]))
    system_message = SystemMessage(content=build_finance_manager_system_prompt(state["company_context"]))
    all_messages = [system_message] + state["messages"]
    response = llm.bind_tools(FINANCE_MANAGER_TOOLS).invoke(all_messages[-15:])
    return {"messages": [response]}


def tools_node(state: FinanceManagerState) -> FinanceManagerState:
    result = ToolNode(FINANCE_MANAGER_TOOLS).invoke(state)
...
def should_continue_tools(state: FinanceManagerState) -> Literal["tools", "end"]:
...
def should_retry_after_tools(state: FinanceManagerState) -> Literal["agent", "end"]:
...
def build_graph():
    graph = StateGraph(FinanceManagerState)
    ...
    try:
        ...
        for msg in reversed(msgs):
            if isinstance(msg, AIMessage) and msg.content:
                ...
                return msg.content
        return "I was unable to process that finance manager request."
    except Exception as exc:
        return f"I encountered an error processing your finance manager request: {exc}"
    finally:
        clear_active_company_context()
